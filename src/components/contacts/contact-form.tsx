...
export function ContactForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!phone.trim()) {
      toast.error(t("contacts.form.errorRequired"));
      return;
    }
    ...
    if (!isEdit && dupMatch?.exact) {
      toast.error(t("contacts.form.errorExists"));
      return;
    }
    ...
    try {
      ...
      if (!accountId) throw new Error(t('common.profileNotLinked'));
      ...
      toast.success(isEdit ? t("contacts.form.successUpdate") : t("contacts.form.successCreate"));
      onOpenChange(false);
      onSaved();
    } catch (err: unknown) {
      ...
      const message = err instanceof Error ? err.message : t("contacts.form.errorFailed");
      toast.error(message);
    } finally {
      setSaving(false);
    }
      ...
      <DialogContent className="bg-popover border-border text-popover-foreground sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-popover-foreground">
            {isEdit ? t("contacts.form.titleEdit") : t("contacts.form.titleAdd")}
          </DialogTitle>
          <DialogDescription className="text-muted-foreground">
            {isEdit
              ? t("contacts.form.descriptionEdit")
              : t("contacts.form.descriptionAdd")}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="cf-phone" className="text-muted-foreground">
              {t("contacts.form.phoneLabel")} <span className="text-destructive">{t("contacts.form.phoneRequired")}</span>
            </Label>
            <Input
              id="cf-phone"
              value={phone}
              ...
              placeholder={t("contacts.form.phoneExample")}
              className="bg-muted border-border text-foreground placeholder:text-muted-foreground"
            />
            {dupMatch ? (
              <div
              ...
              >
                ...
                <div className="space-y-1">
                  <p>
                    {dupMatch.exact
                      ? t("contacts.form.errorExists")
                      : t("contacts.form.errorSimilar")}
                  </p>
                  {onViewExisting && (
                    <button
                      type="button"
                    ...
                    >
                      {t("contacts.form.view", { name: dupMatch.contact.name || dupMatch.contact.phone })}
                    </button>
                  )}
                </div>
              </div>
