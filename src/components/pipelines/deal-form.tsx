...
export function DealForm({
  ...
  async function handleSave() {
    ...
    if (deal) {
      const { error } = await supabase
        ...
        .eq("id", deal.id);
      if (error) {
        toast.error(t("pipelines.failedSaveDeal"));
        setSaving(false);
        return;
      }
    } else {
      ...
      if (error) {
        toast.error(t("pipelines.failedCreateDeal"));
        setSaving(false);
        return;
      }
    }
    ...
    setSaving(false);
    toast.success(deal ? t("pipelines.successUpdate") : t("pipelines.successCreate"));
    onOpenChange(false);
    onSaved();
  }
  ...
  async function handleStatusChange(status: DealStatus) {
    ...
    toast.success(
      status === "won" ? t("pipelines.markedWon") : status === "lost" ? t("pipelines.markedLost") : t("pipelines.markedOpen"),
    );
    onOpenChange(false);
    onSaved();
  }
  ...
  async function handleDelete() {
    ...
    setDeleting(false);
    if (error) {
      toast.error(t("pipelines.failedDelete"));
      return;
    }
        ...
        <div className="flex h-full flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            <div className="grid gap-2">
              <Label className="text-muted-foreground">{t("pipelines.titleLabel")}</Label>
              <Input
                value={title}
                onChange={(e) => setTitle(e.target.value)}
                placeholder={t("common.placeholders.dealTitle")}
                className="border-border bg-muted text-foreground"
              />
            </div>

            <div className="grid gap-2">
              <Label className="text-muted-foreground">{t("pipelines.contactLabel")}</Label>
              <select
                value={contactId}
            ...
            <div className="grid grid-cols-[1fr_110px] gap-3">
              <div className="grid gap-2">
                <Label className="text-muted-foreground">{t("pipelines.valueLabel")}</Label>
              ...
              <div className="grid gap-2">
                <Label className="text-muted-foreground">{t("pipelines.currencyLabel")}</Label>
                <select
                  value={currency}
            ...
            <div className="grid gap-2">
              <Label className="text-muted-foreground">{t("pipelines.expectedCloseDateLabel")}</Label>
              <Input
                type="date"
            ...
            <div className="grid gap-2">
              <Label className="text-muted-foreground">{t("pipelines.stageLabel")}</Label>
              <select
                value={stageId}
            ...
            <div className="grid gap-2">
              <Label className="text-muted-foreground">{t("pipelines.assignedToLabel")}</Label>
              <select
                value={assignedTo}
            ...
            <div className="grid gap-2">
              <Label className="text-muted-foreground">{t("pipelines.notesLabel")}</Label>
              <Textarea
                value={notes}
            ...
            {deal &&
              (confirmDelete ? (
                <div className="mt-3 flex items-center justify-between gap-2 rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs">
                  <span className="text-destructive">{t("pipelines.deleteConfirm")}</span>
