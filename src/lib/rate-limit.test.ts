...
describe("getUpstashRedisMetrics", () => {
  it("returns fallback_in_memory when env vars are unconfigured", async () => {
    const metrics = await getUpstashRedisMetrics();
    expect(metrics.configured).toBe(false);
    expect(metrics.status).toBe("fallback_in_memory");
    expect(metrics.message).toBeDefined();
  });
});
