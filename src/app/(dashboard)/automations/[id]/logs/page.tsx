  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setError(err instanceof Error ? err.message : t("automations.failedLoadLogs"))
      }
    }
    load()
  }, [id])
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">{t("automations.executionLogs")}</p>
        </div>
      </div>

      {logs.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-dashed border-border bg-card/40">
          <p className="text-sm text-foreground">{t("automations.noExecutions")}</p>
          <p className="mt-1 text-xs text-muted-foreground">
            {t("automations.triggerAutomationToSeeRuns")}
          </p>
        </div>
      ) : (
                  ...
                  <div className="border-t border-border px-4 py-3">
                    ...
                    <ul className="space-y-1.5">
                      ...
                      {(log.steps_executed ?? []).length === 0 && (
                        <li className="text-xs text-muted-foreground">{t("automations.noStepsRecorded")}</li>
                      )}
                    </ul>
                  </div>
                )}
    ...
    <li className="flex items-start gap-2 text-xs">
      <span
        className={cn(
          ...
          ok ? "bg-primary-soft text-primary" : "bg-destructive/15 text-destructive",
        )}
        aria-hidden
      >
