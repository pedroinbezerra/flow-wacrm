...
import { AUTOMATION_TEMPLATES, getTemplateDisplay, type TemplateSlug } from "@/lib/automations/templates"
...
export default function NewAutomationPage() {
  ...
  const initial: BuilderInitial = useMemo(() => {
    if (template && AUTOMATION_TEMPLATES[template]) {
      const templateDef = AUTOMATION_TEMPLATES[template]
      const display = getTemplateDisplay(template, t)
      const steps = expandFromSeeds(
        templateDef.steps.map((seed, idx) => ({
          index: idx,
      ...
      return {
        ...
        trigger_type: templateDef.trigger_type,
        trigger_config: templateDef.trigger_config as Record<string, unknown>,
        is_active: false,
        steps,
      }
    }
